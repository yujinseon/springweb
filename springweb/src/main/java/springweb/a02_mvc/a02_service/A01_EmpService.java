...
public class A01_EmpService {
	...
	public ArrayList<Emp> getEmpList(Emp sch) {
		...
		if (sch.getJob() == null)
			sch.setJob("");
		/*
		 * dao.getCount(10); dao.getJobCount("A"); dao.getEmpDetail(7521);
		 * dao.getDeptSal(10); dao.getStaticsEmp("1");
		 * System.out.println("데이터 건수:"+dao.getEnames(10).size()); // service는 현재 dao객체자
		 * 정상적으로 생성되어서 // 사용하는 한지를 테스트 하는 용도로 호출하는 것이다.
		 * System.out.println("데이터 건수(job-empno):"+dao.getEmpnos("SALESMAN").size());
		 * System.out.println("데이터건수(dept-emp):"+ dao.getDeptEmp().size()); public
		 * double getAvg(String job);
		 * 
